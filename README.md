...
│  想象一家全国连锁店：                                                         │
│  - 北京有店，上海有店，广州有店...                                          │
│  - 你在北京买东西，直接从北京店买，不用从广州发货                            │
│  - 这就是 CDN：把内容放在离你最近的服务器上                                  │
│                                                                             │
│  【没有 CDN 的情况】                                                         │
│  ┌──────────┐          ┌──────────┐                                         │
│  │  你的电脑  │────────▶│  目标服务器 │                                        │
...
│                                                                             │
│  【正常网站用 CDN】                                                          │
│  - www.google.com → Google 的 CDN → 离你最近的服务器                        │
│  - api.github.com → GitHub 的 CDN → 离你最近的服务器                        │
│                                                                             │
│  【翻墙用 CDN】                                                              │
│  - 你的节点 → 套 CDN → 流量看起来像访问正常网站                              │
│  - 节点 IP 被隐藏，不容易被封                                               │
│  - 延迟更低，速度更快                                                       │
│                                                                             │
│  【为什么翻墙要套 CDN？】                                                    │
│  1. 隐藏真实 IP - 节点 IP 被 CDN 隐藏                                       │
│  2. 防检测 - 流量看起来像访问 CF 域名（正常网站）                            │
│  3. 防封 - 即使节点被封，换一个域名继续用                                    │
│  4. 加速 - 流量经过 CDN 边缘节点，延迟低                                     │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
...
│  服务商        特点                    用途                                 │
├─────────────────────────────────────────────────────────────────────────────┤
│  Cloudflare    免费，全球节点多         反代，套 CDN（最常用）              │
│  Cloudflare    Argo 隧道               流量走 CF 边缘，不需要开放端口       │
│  Google CDN    全球节点多               优选域名（www.google.com）         │
│  GitHub CDN    全球节点多               优选域名（api.github.com）         │
│  阿里云 CDN    中国节点多               优选域名（gitee.com）              │
│  腾讯云 CDN    中国节点多               优选域名                           │
└─────────────────────────────────────────────────────────────────────────────┘
...
│                                                                             │
│  【Cloudflare 在翻墙里的作用】                                               │
│  1. 反代（Reverse Proxy）- 把节点域名映射到 CF 域名                         │
│  2. 套 CDN - 流量经过 CF 边缘节点，隐藏真实 IP                              │
│  3. Argo 隧道 - 流量走 CF 边缘，不需要开放端口                              │
│  4. Workers - 用 JS 代码反代到真实节点                                      │
│                                                                             │
│  【Cloudflare 免费套餐限制】                                                 │
│  - 反代：无限                                                                │
│  - Workers：10 万次/天                                                       │
...
│  想象一个中介：                                                               │
│  - 你（客户）找中介（反代）买东西                                              │
│  - 中介（反代）帮你从真正卖家（节点）那里买                                    │
│  - 你只知道中介的地址，不知道真正卖家的地址                                    │
│  - 这就是反代：用另一个域名/地址代替真实地址                                  │
│                                                                             │
│  【没有反代】                                                                 │
│  ┌──────────┐          ┌──────────┐          ┌──────────┐                  │
│  │  客户端   │────────▶│  节点真实  │────────▶│  目标网站  │                  │
...
│                                                                             │
│  1. 套 CDN 加速                                                             │
│     - 流量经过 CF CDN 边缘节点，延迟更低                                    │
│     - CF CDN 全球 300+ 节点，就近连接                                        │
│                                                                             │
│  2. 隐藏真实 IP                                                             │
│     - 真实节点 IP 被 CF CDN 隐藏                                            │
│     - 别人只能看到 CF CDN 的 IP，看不到你的节点 IP                           │
│     - 即使被发现，也只是 CF 的 IP，不会封你的节点                            │
│                                                                             │
│  3. 防检测                                                                   │
│     - 流量看起来像访问正常网站（CF CDN 流量）                                │
│     - CF CDN 是合法的，不会被墙                                            │
│     - 检测方看到 CF 流量，不会认为是代理流量                                 │
│                                                                             │
│  4. 防封                                                                     │
│     - 即使节点被封，换一个反代域名继续用                                    │
│     - 节点真实 IP 不变，只是反代域名变了                                    │
...
┌─────────────────────────────────────────────────────────────────────────────┐
│  怎么反代（Cloudflare Workers）                                              │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  步骤 1: 创建 CF Worker                                                      │
│  - 打开 Cloudflare 控制台 → Workers                                         │
│  - 点击 "Create Worker"                                                      │
│  - 输入 Worker 名称                                                          │
│                                                                             │
│  步骤 2: 粘贴反代代码                                                        │
...
│  - 点击 "Deploy"                                                            │
│  - 等待部署完成（约 1 分钟）                                                  │
│  - 复制 Worker 的 URL（如 abc.workers.dev）                                │
│                                                                             │
│  步骤 4: 绑定自定义域名（可选）                                               │
│  - 如果你有域名，可以绑定自定义域名                                          │
│  - Workers → Settings → Custom Domain                                      │
│  - 输入你的域名（如 abc.yourdomain.com）                                    │
│  - 按照提示添加 DNS 记录                                                     │
│                                                                             │
│  步骤 5: 修改节点配置                                                        │
│  - 将反代后的域名填入节点的 CFIP 变量                                        │
│  - 客户端用新域名连接                                                        │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
...
│                                                                             │
│  Q: 反代后节点打不开？                                                       │
│  A: 检查反代域名是否正确，检查 Worker 是否部署成功                           │
│                                                                             │
│  Q: 反代后延迟变高？                                                        │
│  A: CF CDN 可能有延迟，换优选域名试试                                        │
│                                                                             │
│  Q: 反代后哪吒不显示？                                                      │
│  A: 检查 NEZHA_SERVER 是否正确，检查反代域名是否可访问                       │
│                                                                             │
│  Q: 反代域名被墙了怎么办？                                                  │
│  A: 换一个反代域名，重新部署 Worker                                         │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
